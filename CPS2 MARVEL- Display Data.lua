...
--	Friction		  = 229, --LOOOK IN TO THIS!!!
	...
	function validplayer()
		...
		if rb(address) == 0 or rb(address) == nil then
			return 1
		else
			return 2
		end
...
--	if validplayer() == 2 then 	
		...
		--	FloatingPointX
			gui.text(text.x+51,text.y, string.format(".0x%x", PlayerZ[Current_Player].Current_Pos_X_FloatingPoint))
		--	VEL X
		...
		--	FloatingPointY
			gui.text(text.x+51,text.y*2, string.format(".0x%x", PlayerZ[Current_Player].Current_Pos_Y_FloatingPoint))
		--	VEL Y
